...
- The AI program generator now says what went wrong. Every failure inside it reached the browser as Next.js's generic "unexpected response" string, which the screen then printed at the athlete verbatim; the allowance being spent, the model being unavailable and the model returning nothing usable are now three distinct messages, and the real cause is recorded in the server log.
