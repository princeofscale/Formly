...
- Search suggestions and push notification copy moved off the reasoning model onto a fast one. Neither judges training data — one picks rows out of a numbered catalog, the other writes a single sentence — and measured side by side on the same prompts the fast model returned the same exercises in 3.5-4.5s instead of 16-27s, and a better push line in 2-3s instead of 20-48s. It also spent roughly a thirtieth of the output tokens, because reasoning is billed as completion. The search timeout came down from 25s to 15s to match. Debriefs, the coach thread, exercise swaps, the dashboard card and the program generator still use the reasoning model, where the judgement is the product.
- Every gateway call now logs which screen asked, which model answered, how long it took and what it spent. The move to a reasoning model was decided on a handful of hand-run measurements because there was nothing in production to consult; there is now. Prompts and replies stay out of the log — both carry training history.
- Publishing a release now asks the deployment for four pages before the workflow goes green. Vercel calls a deployment ready once the build finishes, so a release that built and then failed on every request looked exactly like a good one. The check follows the sign-in redirect and confirms each page is the page it asked for, because middleware answers any unknown path with the sign-in screen and a 200.
...
- A reply wrapped in a markdown code fence no longer breaks the screen that asked for it. Asking the gateway for a JSON object is a request, not a guarantee: several models answer with the object fenced anyway, and all seven AI surfaces parse that text as JSON — two of them surfacing the failure to the athlete as an error. The fence now comes off once, in the client, so no surface has to know about it.
