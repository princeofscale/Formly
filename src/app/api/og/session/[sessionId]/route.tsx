    ...
    (
      <div
        style={{
          ...
          background: 'radial-gradient(circle at 0% 0%, rgba(255,196,68,0.18), transparent 55%), radial-gradient(circle at 100% 100%, rgba(167,139,250,0.18), transparent 55%), #0A0A0E',
          color: 'white',
          padding: 60,
      ...
      >
        ...
        <div style={{ fontSize: 26, opacity: 0.45, marginTop: 6, letterSpacing: 4, textTransform: 'uppercase' }}>
          Tonnage moved
        </div>
        ...
        <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: 10 }}>
          {summary.topExercises.slice(0, 3).map(ex => (
            <div key={ex.exerciseId} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 26 }}>
...
function Stat({ label, value, highlight }: StatProps) {
  return (
    ...
    >
      <span style={{ fontSize: 18, letterSpacing: 3, opacity: 0.45, textTransform: 'uppercase' }}>{label}</span>
      <span
        style={{
          fontSize: 56,
          fontWeight: 900,
