    ...
    },
    {
      ...
      body: (
        <p>
          ...
          тренировок, ответы тренера, подсказки поиска и тексты уведомлений — формируются сторонней
          большой языковой моделью (Grok 4.5, доступ через шлюз CheapVibeCode). Такие модели
          допускают фактические ошибки, в связи с чем полученные материалы следует рассматривать как
          предположение, подлежащее самостоятельной проверке. Модели передаются только обезличенные
          сводные показатели тренировок; адрес электронной почты, фотографии и личные сообщения не
          передаются. Количество обращений к функциям искусственного интеллекта ограничено суточной
          квотой.
        </p>
      ),
    },
...
function termsEn(mailto: React.ReactNode, operator: string, jurisdiction: string): LegalSection[] {
  return [
    ...
    },
    {
      ...
      body: (
        <p>
          Certain material — session debriefs, recommendations, programs, session titles, coach
          replies, search suggestions and notification copy — is generated by a third-party large
          language model (Grok 4.5, reached through the CheapVibeCode gateway). Such models produce
          factual errors, so their output should be treated as a suggestion to be verified rather
          than as fact. Only pseudonymised aggregate training figures are transmitted; email
