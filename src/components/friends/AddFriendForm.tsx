  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        >
          {pending ? '...' : result?.ok ? <Check className="h-4 w-4" /> : <><UserPlus className="h-4 w-4" />{t('add')}</>}
        </button>
      </div>
      {result && !result.ok && result.errorKey && (
        <p className="text-xs" style={{ color: '#FF6E76' }}>{t(`addError.${result.errorKey}`)}</p>
      )}
      {result?.ok && (
        <p className="text-xs" style={{ color: '#22D3A8' }}>{t('addSuccess')}</p>
      )}
    </form>
  )
}
